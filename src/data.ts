...
export type Composition = {socialProtectionSources?:Source[];socialProtection?:{year:string;sourceId?:string;total:number;items:{name:string;value:number}[]}[];history?:import('./budgetMath').FunctionalHistory;schemaVersion:number;asOf:string;unit:string;basis:string;sources:Source[];years:{year:string;total:number;debtInterest?:number;items:{name:string;value:number}[]}[];notes:string[]};
